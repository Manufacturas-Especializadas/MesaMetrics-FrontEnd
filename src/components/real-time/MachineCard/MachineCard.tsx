...
import { type MachineMetrics } from "@/api/services/MetricsService"; // Asegúrate de importar la interfaz
...
export const MachineCard: React.FC<StatProps> = ({ metrics, isHistorical = false, realTimeId }) => {
    ...
    return (
        <article className={`flex flex-col rounded-xl border shadow-sm overflow-visible transition-all duration-500 ${styles.container} ${styles.opacity}`}>
                ...
                <div className="grid grid-cols-5 md:grid-cols-5 bg-white">
                    ...
                    <div className="">
                        <button
                            ...
                            onClick={() => realTimeId && navigate(`/editar-maquina/${realTimeId}`)}
                        >
                            <Settings />
                        </button>
                    </div>
